...
while True:
    consulta = input('''¿Qué producto desea agragar al sock?
    introduzca FIN si no desea introducir nada mas:''')
    if consulta == 'FIN':
        break
    elif consulta in stock:
        cantidad = int(input('Ingrese la cantidad de su producto:'))
        stock[consulta] += cantidad
    else:
        print('No hay stock del producto, continúe con otro')

for k, v in stock.items():
        print(f'Cantidad de {k}: {v}u.')
